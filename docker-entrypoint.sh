...
#  params: 
#   1: site - the site path e.g /var/www/html/mediawiki 
#
checkWikiDB() {
  # get parameters
  local l_site="$1"
  # where are the MediaWiki local settings? 
  local l_settings="$l_site/LocalSettings.php"
...
prepare_mediawiki() {
  ln -s $mwpath $apachepath/mediawiki
	cat << EOF > $mwpath/LocalSettings.php
<?php
